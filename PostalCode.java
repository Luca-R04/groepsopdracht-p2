public class PostalCode {
    ...
    public static String formatPostalCode(/* non_null */ String postalCode) {
        ...
        } else if (Integer.valueOf(postalCode.trim().substring(0, 4)) > 999 && Integer.valueOf(postalCode.trim().substring(0, 4)) <= 9999 
                && postalCode.trim().substring(4).trim().length() == 2 && Character.isLetter(postalCode.trim().substring(4).trim().toUpperCase().charAt(0))) {
            return postalCode.trim().substring(0, 4) + " " + postalCode.trim().substring(4).trim().toUpperCase();
        }

        throw new IllegalArgumentException("Postal Code out of bounds");
    }
}
